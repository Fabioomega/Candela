...
impl<'a, T> MutIter<'a, T> {
    ...
    pub fn for_each(self, mut f: impl FnMut(&mut T)) {
        if self.layout.is_contiguous() {
            for el in
                self.data[self.layout.offset()..self.layout.offset() + self.layout.len()].iter_mut()
            {
                f(el);
            }
        ...
        if stride == 1 {
            for _ in 0..left_over {
                for el in self.data[pos..pos + n].iter_mut() {
                    f(el);
                }
            ...
            for _ in 0..left_over {
                ...
                for _ in 0..n {
                    ...
                    // SAFETY: a well-formed layout only ever visits in-bounds
                    // positions of its own buffer, so `pos_inner` is a valid
                    // index. Dropping the bounds check keeps the strided write
                    // from stalling memory-level parallelism on gather-heavy
                    // layouts (e.g. transposed).
                    f(unsafe { self.data.get_unchecked_mut(pos_inner) });
...
impl<'a, T: Clone> ExactSizeIterator for IterWith<'a, T> {}

impl<'a, T: Clone> FusedIterator for IterWith<'a, T> {}
