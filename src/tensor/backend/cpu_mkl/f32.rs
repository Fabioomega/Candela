...
use crate::tensor::backend::cpu_mkl::mkl_extension::cblas_sgemm_batch_strided;
use crate::tensor::mem_formats::layout::Layout;
use crate::tensor::ops::def_op::{OpKind, OpKindScalar, Sign};
use crate::tensor::storage::{Storage, TensorData};
use intel_mkl_sys::{vsExp, vsLn, vsLog2, vsTanh};
use wide::f32x16;
...
fn compute_out(op: &OpKindScalar<f32>, src: &[f32], dst: &mut [f32]) {
    match op {
        ...
        OpKindScalar::Exp => {
            unsafe { vsExp(dst.len() as i32, src.as_ptr(), dst.as_mut_ptr()) };
        }
        OpKindScalar::Ln => {
            unsafe { vsLn(dst.len() as i32, src.as_ptr(), dst.as_mut_ptr()) };
        }
        OpKindScalar::Log2 => {
        ...
        OpKindScalar::Tanh => {
            unsafe { vsTanh(dst.len() as i32, src.as_ptr(), dst.as_mut_ptr()) };
        }
    }
}
...
fn compute_inplace(op: &OpKindScalar<f32>, dst: &mut [f32]) {
    match op {
        ...
        OpKindScalar::Exp => {
            unsafe { vsExp(dst.len() as i32, dst.as_ptr(), dst.as_mut_ptr()) };
        }
        OpKindScalar::Ln => {
            unsafe { vsLn(dst.len() as i32, dst.as_ptr(), dst.as_mut_ptr()) };
        }
        OpKindScalar::Log2 => {
        ...
        OpKindScalar::Tanh => {
            unsafe { vsTanh(dst.len() as i32, dst.as_ptr(), dst.as_mut_ptr()) };
        }
    }
}
