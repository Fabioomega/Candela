...
### 6. Crate root doc - `//!` in `lib.rs`

**Who reads this:** Someone who just added `candela` to their `Cargo.toml` and
opened docs.rs. They are an API consumer looking for the entry point, not a
contributor reading the project's story.

The crate root is a *third* register, distinct from the other two: it is neither
the README's project pitch nor a `doc/*.md` deep-dive. It orients an API user and
then gets out of the way. Keep it to three things:

- **One tight "what is this" paragraph.** What the crate does and the one idea a
  user must hold to use it (here: ops are lazy, `.materialize()` runs them).
...
license, build-from-source - that is noise on an API reference, and its
...
The precise, declarative register of §1 - this is reference material - with a
single worked example. No project narrative.
